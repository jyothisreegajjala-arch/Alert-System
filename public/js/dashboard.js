...
function renderActiveSOSTracker(emergency) {
  ...
  trackerEl.innerHTML = `
    <div class="active-sos-card">
      ...
      <p style="margin-top:0.5rem; color:#cbd5e1;">Triggered at ${emergency.time} on ${emergency.date} (${emergency.address})</p>
  ...
  card.innerHTML = `
    <div class="alert-card-header">
      <div>
        ...
        <p style="font-size:0.85rem; color:var(--dark-muted);">Alert ID: #${emergency.alertId} • ${emergency.date} ${emergency.time}</p>
      </div>
      <div>${badge}</div>
    </div>
    ...
    return `
      <div class="notif-card ${cardClass}">
        ...
        <div class="notif-meta-grid">
          ...
          <div><strong style="color:#94a3b8;">Date & Time:</strong><br><span style="color:#fff; font-weight:700;">${n.date || 'Recent'} ${n.time || ''}</span></div>
        </div>
